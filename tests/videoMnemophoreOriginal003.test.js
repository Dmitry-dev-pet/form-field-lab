...
const asset = name => new URL(`../public/videos/mnemophore-original-003/${name}`, import.meta.url);

test("Original 003 packages the exact stateful Mnemophore root", () => {
  ...
  assert.equal(MNEMOPHORE_CORE_GENOME.length, 271);
  assert.equal(genome, MNEMOPHORE_CORE_GENOME);
  assert.equal(manifest.series, "Form / Field Originals");
  assert.equal(manifest.number, 3);
  assert.equal(manifest.genome.characters, 271);
  assert.equal(manifest.genome.stateSurvivesFrame, true);
  ...
  assert.match(caption, /form=mnemophore/);
});

test("Original 003 proves two independent renders are byte-identical", () => {
...
test("Original 003 includes a TikTok-ready deterministic MP4 and cover", () => {
  const video = readFileSync(asset("mnemophore-original-003.mp4"));
  const cover = readFileSync(asset("cover.png"));

  assert.ok(statSync(asset("mnemophore-original-003.mp4")).size > 1_000_000);
