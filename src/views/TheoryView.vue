      ...
      <article id="torophore" class="theory-card pelagion-theory chronophore-theory">
        ...
        <div class="theory-body pelagion-theory-grid">
          <div>
            ...
            <p>Центры лежат строго в плоскости <code>z = 0</code>. Третье измерение действительно присутствует внутри каждого примитива <code>torus</code>, а не в траектории колонии. Лабораторный параметр <code>B</code> меняет только малый радиус: волна толщины идёт вместе с фазой и не сдвигает центры. При <code>B = 0</code> второй аргумент удаляется из RAW, и код снова становится точным источником.</p>
            <div class="math-scroll">
              \[
              \begin{aligned}
              g_i^{(n)}&amp;=\frac{255}{2}\left(1-\sin d_i^{(n)}\right),\\
          ...
          <div class="pelagion-genome">
            ...
            <p>Пять основных ползунков меняют <code>C += 2</code>, предел цикла, множитель <code>TAU * 2</code>, большой радиус и амплитуду дыхания. В точной настройке один дополнительный параметр растягивает аргумент шума. Дыхание добавляет компактный второй аргумент <code>torus()</code>; цветных палитр, глубинной волны центров и ориентационных мутаций по-прежнему нет.</p>
            <div class="pelagion-links">
